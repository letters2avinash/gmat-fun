...
export default function VerbalPracticePage() {
  ...
  const [questionType, setQuestionType] = useState<string>("critical_reasoning");
  ...
  const loadTopics = useCallback(async (qType: string) => {
    ...
    const res = await fetch(
      `/api/test/topics?section=verbal&questionType=${encodeURIComponent(qType)}`
    );
  ...
  async function start() {
    ...
    if (!user) {
      router.replace(`/auth?next=${encodeURIComponent("/practice/verbal")}`);
      return;
    }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-16">
      <div className="max-w-lg w-full">
        <p className="text-xs font-semibold uppercase tracking-wide text-brand mb-2">Verbal Reasoning</p>
        <h1 className="text-3xl font-extrabold tracking-tight mb-2">Practice Verbal</h1>
